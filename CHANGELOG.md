...
Historical incomplete release candidate. The immutable `v0.1.0` tag contains
`0.1.0.dev0` and preserves the E0-E9 implementation record below, but it did
not meet the historical global Definition of Done: 90% branch coverage and a
clean-clone test run below 60 seconds. Its PyPI Trusted Publishing workflow
failed because Trusted Publishing was not configured, so it did not publish to
PyPI. See [ADR 0027](docs/adr/0027-e21-release-truth.md) for the release
record and the next-prerelease criteria.
...
- `docs/traceability.md` (the spec-section -> test generator described in
  ...
  **RESOLVED 2026-07-23:** see the traceability entrega above and
  ...
  CI.
- The `v0.1.0` PyPI Trusted Publishing workflow failed because Trusted
  Publishing was not configured; it did not publish to PyPI. The immutable
  tag must not be moved, recreated, or pushed again to retry publication.
- No demo GIF/asciinema recording is embedded in the README; `asciinema`
  ...
  to render.
- Historical test evidence did not establish a single clean-clone run meeting
  both §0 targets: 90% branch coverage and a run below 60 seconds. Those
  targets remain unweakened and unclaimed debt; current measured quality gates
  are recorded in [ADR 0027](docs/adr/0027-e21-release-truth.md).
